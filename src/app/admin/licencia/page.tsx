...
function formatMonto(monto: number, moneda: string) {
  return new Intl.NumberFormat("es-VE", {
    style: "currency",
    currency: moneda === "CLP" ? "CLP" : moneda === "EUR" ? "EUR" : "USD",
...
function formatPeriodo(inicio: Date, fin: Date) {
  if (inicio.getUTCMonth() === fin.getUTCMonth() && inicio.getUTCFullYear() === fin.getUTCFullYear()) {
    const label = inicio.toLocaleDateString("es-VE", { month: "long", year: "numeric", timeZone: "UTC" })
    return label.charAt(0).toUpperCase() + label.slice(1)
  }
  return (
    inicio.toLocaleDateString("es-VE", { day: "numeric", month: "short", timeZone: "UTC" }) +
    " – " +
    fin.toLocaleDateString("es-VE", { day: "numeric", month: "short", year: "numeric", timeZone: "UTC" })
  )
}
...
export default async function LicenciaPage() {
  ...
  const [lic, pagos] = await Promise.all([
    getLicenciaStatus(),
    prisma.pagoLicencia.findMany({ orderBy: { fechaPago: "desc" }, take: 12 }),
  ])
    ...
    <div className="max-w-xl space-y-6">
              ...
              <div
                ...
                style={{
                  ...
                  background: lic.diasRestantes <= 7
                    ...
                    : `linear-gradient(90deg, ${BRAND.colorDark}, ${BRAND.color})`,
                }}
              />
            </div>
          </div>
      ...
      {lic.suspendida ? (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-sm text-red-700 space-y-1">
          ...
          <p className="font-medium"><a href={`mailto:${contacto}`} className="underline">{contacto}</a></p>
        </div>
      ) : lic.mostrarBanner ? (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-700 space-y-1">
          ...
          <p>Contacto: <a href={`mailto:${contacto}`} className="underline font-medium">{contacto}</a></p>
        </div>
      ) : lic.existe ? (
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm text-gray-600">
          ...
          <p className="mt-1"><a href={`mailto:${contacto}`} className="underline text-blue-500">{contacto}</a></p>
        </div>
      )}
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
        ...
        <div className="space-y-1 text-xs text-gray-500 font-mono">
          ...
          <p>App: <span className="text-gray-700">psicoallameysanz.com</span></p>
        </div>
      </div>
    </div>
  )
