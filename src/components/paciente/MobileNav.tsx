...
export default function MobileNav({ consentimientoFirmado = true }: { consentimientoFirmado?: boolean }) {
      ...
      {visible.map((item) => {
        ...
        if (!consentimientoFirmado) {
          return (
            <div
              key={item.href}
              className="flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl text-center min-w-0 opacity-30 cursor-not-allowed"
            >
              <item.icon size={20} />
